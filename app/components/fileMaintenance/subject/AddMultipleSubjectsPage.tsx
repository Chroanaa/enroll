...
const AddMultipleSubjectsPage: React.FC<AddMultipleSubjectsPageProps> = ({
  ...
  const [focusedFixedAmountRowId, setFocusedFixedAmountRowId] = useState<
    string | null
  >(null);
  ...
  const validateRows = (): { valid: boolean; errors: string[] } => {
    ...
    rows.forEach((row, index) => {
      ...
      if (row.code?.trim() && row.name?.trim()) {
        const hasCredits =
          (Number(row.units_lec) || 0) > 0 || (Number(row.units_lab) || 0) > 0;
        const hasHours =
          (Number(row.lecture_hour) || 0) > 0 ||
          (Number(row.lab_hour) || 0) > 0;
      ...
      if (
        ...
        if (isNaN(amount) || amount < 0) {
          errors.push(
            `Row ${rowNum}: Fixed Amount must be a valid non-negative decimal number`,
          );
        }
      }
    });
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const validRows = rows.filter((row) => {
      ...
      const hasCredits =
        (Number(row.units_lec) || 0) > 0 || (Number(row.units_lab) || 0) > 0;
      const hasHours =
        (Number(row.lecture_hour) || 0) > 0 || (Number(row.lab_hour) || 0) > 0;
  ...
  const performSave = async () => {
    ...
    try {
      ...
      const subjectsData: Omit<Subject, "id">[] = validRows.map((row) => ({
        ...
        fixedAmount:
          row.fixedAmount && row.fixedAmount !== ""
                          ...
                          <div className='relative'>
                            <select
                              ...
                              style={{
                                border: getActivePreset(row)
                                  ...
                                  : "1px solid #E5E7EB",
                                outline: "none",
                                color: getActivePreset(row)
                                  ? colors.secondary
                                  : "#6B5B4F",
                                backgroundColor: getActivePreset(row)
                                  ? `${colors.secondary}10`
                                  : "white",
                                fontWeight: getActivePreset(row)
                                  ? "600"
                                  : "400",
                              }}
                              onFocus={(e) => {
                            ...
                            >
                              ...
                              <option value='lecture'>
                                {presets.lecture.label} (
                                {presets.lecture.description})
                              </option>
                              <option value='lab'>
                            ...
                            <input
                              ...
                              value={
                                focusedFixedAmountRowId === row.id
                                  ...
                                  : row.fixedAmount !== undefined &&
                                      row.fixedAmount !== null
                                    ? Number(row.fixedAmount).toLocaleString(
                                        "en-US",
                                        {
