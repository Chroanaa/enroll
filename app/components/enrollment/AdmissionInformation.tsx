...
import { GraduationCap, Calendar, UserCircle, Building2, BookOpen, Upload, Image as ImageIcon } from "lucide-react";
      ...
      >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="space-y-4">
            <div>
              <label className="flex items-center gap-2 text-sm font-semibold mb-2" style={{ color: colors.primary }}>
            ...
            <div>
              <label className="flex items-center gap-2 text-sm font-semibold mb-3" style={{ color: colors.primary }}>
                <Calendar className="w-4 h-4" style={{ color: colors.secondary }} />
                Term
              </label>
              <div className="flex gap-4">
                {["First", "Second", "Summer"].map((term) => (
                  <label 
                    ...
                    style={{ 
                      borderColor: formData.term === term.toLowerCase() ? colors.secondary : colors.tertiary + "40",
                      backgroundColor: formData.term === term.toLowerCase() ? colors.accent + "15" : "transparent"
                    }}
                  >
                    <input
                      type="radio"
                    ...
                    />
                    <span className="text-sm font-medium" style={{ color: formData.term === term.toLowerCase() ? colors.secondary : colors.primary }}>
                      {term}
                    </span>
                  </label>
                ))}
          ...
          <div className="space-y-4">
            ...
            <div>
              <label className="flex items-center gap-2 text-sm font-semibold mb-3" style={{ color: colors.primary }}>
                <ImageIcon className="w-4 h-4" style={{ color: colors.secondary }} />
                Photo
              </label>
              <div 
              ...
              >
                <div
                  className={`w-32 h-40 sm:w-36 sm:h-44 border-2 border-dashed rounded-lg relative overflow-hidden cursor-pointer transition-all duration-200 group ${
                    photoPreview ? '' : 'flex flex-col items-center justify-center'
                  }`}
                  style={{ 
                    borderColor: photoPreview ? colors.tertiary + "60" : colors.accent + "60",
                    backgroundColor: photoPreview ? '#ffffff' : colors.accent + "05"
                  }}
                  onMouseEnter={(e) => {
                    if (!photoPreview) {
                ...
                >
                  {photoPreview ? (
                    <>
                      ...
                      <div 
                        className="absolute inset-0 bg-black opacity-0 group-hover:opacity-30 transition-opacity duration-200 flex items-center justify-center pointer-events-none z-20"
                      >
                    ...
                    <div className="text-center p-2">
                      <ImageIcon className="w-6 h-6 mx-auto mb-1 group-hover:scale-110 transition-transform duration-200" style={{ color: "#9CA3AF" }} />
