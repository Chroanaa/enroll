...
interface FacultyTeachingLoadProps {
  facultyId: number;
  academicYear: string;
  semester: string;
    ...
    async function fetchAndOpen() {
      ...
      try {
        const res = await fetch(
          `/api/class-schedule?facultyId=${facultyId}&academicYear=${encodeURIComponent(academicYear)}&semester=${encodeURIComponent(semester)}`
        );
        if (res.ok) {
      ...
      const facultyName = faculty
        ? `${faculty.last_name.toUpperCase()}, ${faculty.first_name}${mid}`
        : '';
      const isFullTime = faculty?.position?.toLowerCase().includes('full') ?? false;

      // ── CSS shorthand for table cells ──
      ...
      const tableRows = TIME_SLOTS.map((slotStart, si) => {
        const cells = DAYS.map((_, di) => {
          ...
          return `<td rowspan="${rowspan}" style="${TD}background:${col.bg};color:${col.text};text-align:center;vertical-align:middle;padding:1px;">
            ...
            ${faculty ? `<div style="font-size:7px;line-height:1.1;">${esc(faculty.last_name)}</div>` : ''}
          </td>`;
        }).join('');
...
<head>
  <meta charset="UTF-8"/>
  <title>Confirmation of Teaching Load — ${esc(facultyName)}</title>
  <style>
