  ...
  try {
    ...
    const rows = await prisma.$queryRaw<any[]>`
      WITH approved_petitions AS (
        ...
        WHERE LOWER(COALESCE(psr.status, '')) = 'approved'
      ...
      petition_with_assignment AS (
        SELECT
          ...
          EXISTS (
            ...
            WHERE ss.student_number = ap.student_number
              AND ss.academic_year = ap.academic_year
              AND (
                (ap.semester = 1 AND LOWER(COALESCE(ss.semester, '')) IN (${firstSemesterAliases[0]}, ${firstSemesterAliases[1]}, ${firstSemesterAliases[2]}, ${firstSemesterAliases[3]}))
                OR
                (ap.semester = 2 AND LOWER(COALESCE(ss.semester, '')) IN (${secondSemesterAliases[0]}, ${secondSemesterAliases[1]}, ${secondSemesterAliases[2]}, ${secondSemesterAliases[3]}))
              )
              AND cs.curriculum_course_id = ap.curriculum_course_id
          ) AS has_assignment
        FROM approved_petitions ap
      )
      SELECT
        ...
        pwa.academic_year,
        pwa.semester,
        COUNT(DISTINCT pwa.student_number)::int AS approved_students,
