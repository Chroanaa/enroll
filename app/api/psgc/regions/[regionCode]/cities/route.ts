...
export async function GET(
  request: NextRequest,
  { params }: { params: Promise<{ regionCode: string }> | { regionCode: string } }
) {
  try {
    ...
    const apiRegionCode = regionCode.length === 9 ? `${regionCode}0` : regionCode;
    const response = await fetch(`${PSGC_API_BASE}/v2/regions/${apiRegionCode}/cities`, {
